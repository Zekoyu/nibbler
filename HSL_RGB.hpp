...
/**
 * @brief Rotates <angle> degrees the given RGB values and returns the new values
 * r,g,b should be in the range [0-255]
 * angle should be in the range [0-360]
 */
RGB rotateRGBUsingHSL(int r, int g, int b, int angle)
{
	HSL hsl = rgb2hsl((int) r, (int) g, (int) b);
	hsl.h += (float) angle / 360.0f;
	if (hsl.h > 1.0f)
		hsl.h -= 1.0f;
